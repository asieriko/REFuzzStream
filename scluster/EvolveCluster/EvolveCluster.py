...
from scluster.EvolveCluster.functions import kmedoids
from scluster.EvolveCluster.functions import IC_av
...
def generic_csv(input_file, dimension, chunk_size, tau):
	...
	medoids = []
	for i in range(1,5):
		test = data[data['class'] == i]
		D_test = calculate_distances(test.iloc[:,:dimension].to_numpy(copy=True))
		medoid = np.argmin(D_test.sum(axis=0))
		medoid = int(test.iloc[medoid].name)
		data.loc[data['class'] == i, 'clusters'] = medoid
		medoids.append(medoid)
	...
	new_data = []
	for i in range(1,length):
		new_data.append(all_data[i*chunk_size,(i+1)*chunk_size].iloc[:,:dimension].to_numpy(copy=True))

	a.cluster(new_data)
	# pickle.dump(a, open('results/rbfgenerator/%s-dim_results.pkl'%(seed_dim), 'wb'))
	a.to_csv_new_synthetic('rbfgenerator/',dimension)
...
if __name__ == '__main__':
	# main()
	# cover_experiment()
	generic_csv("/home/asier.urio/Ikerketa/Projects/DataStream/datasets/DS1norm.csv",2,100,0.5)
